...
namespace _11._Searching
{
    internal class Graph
    {
		 ...
		 * 간선의 방향성에 따라 단방향 그래프, 양방향 그래프가 있음             // 그래프를 그릴 때 단방향만 화살표를 그려주는게 룰국룰
        ...
        // bool[시작정점, 끝정점]의 방식으로 넣어준다   예) [2, 4] 2정점 -> 4정점
        ...
        // 단방향은 대칭이 아니며 양방향일시 대칭이 된다
        public void Test()
        {
            // 만약 그래프에서 3번에서 1번 가는게 가능한가
            if (matrixGraph[3, 1]) { }
        }

        // 예시 - 양방향 연결 그래프
        bool[,] matrixGraph1 = new bool[5, 5]           // 정점이 5개있다.
        {
        ...
        // const 상수이며 INF를 외부에서 바뀔 수 있음
        // 최대값이면 단절되어있자고 하기위해 최대값을 만들고
        const int INF = int.MaxValue;

        // 예시 - 단방향 가중치 그래프 (단절은 최대값으로 표현하거나, 음수-1fh 표현)
        ...
        // 장점 : 메모리 사용량이 적음   O(N)
        // 단점 : 인접여부를 확인하기 위해 리스트 탐색이 필요 (탐색을 요구하므로 느릴 수 있다)   O(N)
