...
{
    internal class Program<TKey,TValue>
    {
        /******************************************************
		 * 해시테이블 (HashTable)
		 * 
        ...
        // -대표적인 충돌 해결방안으로 "체이닝"과 "개방주소법"이 있음-
        ...
        // 재해싱 : 해시테이블의 크기를 늘리고 테이블 내의 모든 데이터를 다시 해싱 ---- 재해싱을 안해주는 것보단 낫다

        static void Dictionary()
        {
            Dictionary<string, Item> dictionary1 = new Dictionary<string, Item>();

            // 해당 키를 추가
            dictionary1.Add("초기아이템", new Item("초보자용 검", 10));
            dictionary1.Add("초기방어구", new Item("초보자용 가죽갑옷", 30));
            dictionary1.Add("전직아이템", new Item("푸른결정", 1));
            dictionary1.Add("슬라임조각", new Item("슬라임조각", 3));

            // 탐색
            Console.WriteLine(dictionary1["초기아이템"].name);

            // 접근
            dictionary1.Remove("전직아이템");

            // 확인
            if (dictionary1.ContainsKey("초기아이템"))
            {
                Console.WriteLine("딕쳐너리에 초기아이템이 있음");
            }
